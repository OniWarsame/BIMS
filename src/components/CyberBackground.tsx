...
export default function CyberBackground() {
  ...
  useEffect(() => {
    ...
    /* Particles — cyan & orange from image */
    ...
    const draw = () => {
      ...
      SCANLINES.forEach((panel, pi) => {
        ...
        [[px,py,1,1],[px+pw,py,-1,1],[px,py+ph,1,-1],[px+pw,py+ph,-1,-1]].forEach(([bx,by,dx,dy])=>{
          ...
          ctx.save(); ctx.shadowBlur=proximity>0.2?12:0;
        ...
        ctx.letterSpacing="0.2em"; ctx.fillText(panel.label,px+4,py-6); ctx.letterSpacing="0";
      ...
      PARTICLES.forEach(p => {
        ...
        if(prx>0.05){ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(M.x,M.y);ctx.strokeStyle=`hsla(${p.hue},100%,65%,${prx*0.2})`;ctx.lineWidth=prx*0.8;ctx.stroke();}
      ...
      if(glitching&&imgLoaded){
        ...
        for(let s=0;s<slices;s++){const gy=Math.random()*H,gh=2+Math.random()*8,gx=(Math.random()-0.5)*18;ctx.save();ctx.drawImage(canvas,0,gy,W,gh,gx,gy,W,gh);ctx.restore();}
        ctx.save();ctx.globalAlpha=0.15;ctx.globalCompositeOperation="screen";
        const cy2=Math.random()*H;ctx.fillStyle="rgba(255,0,80,0.6)";ctx.fillRect(-4,cy2,W,3+Math.random()*6);
        ctx.fillStyle="rgba(0,255,255,0.6)";ctx.fillRect(4,cy2+2,W,3+Math.random()*6);ctx.restore();
      }
      ...
      if(M.x>0){
        ...
        ctx.save();ctx.shadowBlur=12;ctx.shadowColor="rgba(0,230,255,0.95)";
        ...
        ctx.shadowBlur=16;ctx.beginPath();ctx.arc(MX,MY,M.click?1.5:2.2,0,Math.PI*2);
        ctx.fillStyle="rgba(0,245,255,0.96)";ctx.fill();ctx.restore();
      }
