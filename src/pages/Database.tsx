...
const LockScreen = ({ onUnlock, onBack }: { onUnlock: () => void; onBack: () => void }) => {
  ...
  const fpColor  = scanState==="error"?"hsl(0,85%,62%)":scanState==="preview"||scanState==="granting"?"hsl(192,100%,58%)":scanState==="scanning"?"hsl(192,100%,65%)":"hsl(195,80%,45%)";
    ...
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden">
      ...
      <div className="fixed top-0 left-0 right-0 z-20 flex items-center px-4 py-3 cyber-header">
        <button onClick={onBack}
          style={{display:"flex",alignItems:"center",gap:8,fontFamily:"'Orbitron',sans-serif",fontSize:10,fontWeight:700,letterSpacing:"0.14em",color:"hsl(192,100%,72%)",background:"none",border:"none",cursor:"pointer",padding:"6px 10px",borderRadius:8,transition:"all .18s"}}
      ...
      <div className="fixed left-3 top-1/2 -translate-y-1/2 z-10 flex flex-col gap-2.5 pointer-events-none">
        {sideLeft.map((item,i)=>(
          <motion.div key={i}
            ...
            style={{background:"rgba(1,10,24,0.85)",border:"1px solid rgba(50,190,218,0.18)",backdropFilter:"blur(12px)",borderRadius:8,padding:"8px 12px",minWidth:150}}>
            <motion.div animate={{opacity:[0.35,0.7,0.35]}} transition={{duration:2.5+i*0.4,repeat:Infinity,delay:i*0.3}}>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:9,fontWeight:700,color:"hsl(192,100%,65%)"}}>{item.val}</div>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      <motion.div initial={{opacity:0,y:28,scale:0.97}} animate={{opacity:1,y:0,scale:1}} transition={{duration:0.35}}
        ...
        <h1 style={{fontFamily:"'Orbitron',sans-serif",fontSize:18,fontWeight:900,letterSpacing:"0.18em",
          color:"hsl(192,100%,72%)",textShadow:"0 0 20px rgba(50,190,218,0.8)",marginBottom:4}}>
          DATABASE ACCESS
        </h1>
        ...
        <div style={{position:"relative",display:"inline-block",marginBottom:20}}>
          ...
          <div className={scanState==="scanning"?"scanner-pulse":""} style={{width:220,height:220,borderRadius:"50%",
            background:scanState==="error"?"radial-gradient(circle at 38% 38%,hsla(0,70%,8%,0.97),hsla(0,70%,3%,0.99))":"radial-gradient(circle at 38% 38%,rgba(4,18,32,0.97),rgba(1,10,24,0.99))",
            ...
            {scanState==="granting" ? (
              <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type:"spring",stiffness:300}}>
                <CheckCircle style={{width:60,height:60,color:"hsl(192,100%,65%)",filter:`drop-shadow(0 0 18px ${glowCol})`}}/>
              </motion.div>
            ) : (
            ...
            <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}}
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
                ...
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <motion.div animate={{opacity:[1,0.3,1]}} transition={{duration:1,repeat:Infinity}}
                    style={{width:7,height:7,borderRadius:"50%",background:"hsl(192,100%,58%)",boxShadow:"0 0 8px rgba(50,190,218,0.8)"}}/>
                ...
                <span style={{fontFamily:"'Orbitron',sans-serif",fontSize:14,fontWeight:900,color:"hsl(192,100%,65%)"}}>{countdown}</span>
              </div>
              <div style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:44,height:44,borderRadius:10,background:"rgba(44,178,212,0.12)",border:"1px solid rgba(50,190,218,0.3)",
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:800,color:"hsl(192,100%,65%)",flexShrink:0}}>
              ...
              <div style={{height:3,margin:"0 14px 12px",borderRadius:2,background:"rgba(44,178,212,0.12)",overflow:"hidden"}}>
                <motion.div style={{height:"100%",background:"linear-gradient(90deg,hsl(192,100%,48%),hsl(200,100%,60%))",originX:0}}
        ...
        {(scanState==="idle"||scanState==="error") && (
          <motion.button onClick={startScan}
            ...
            style={{width:"100%",height:46,borderRadius:13,display:"flex",alignItems:"center",justifyContent:"center",gap:10,
              ...
              background:"linear-gradient(135deg,hsl(192,100%,45%),hsl(200,100%,58%))",
