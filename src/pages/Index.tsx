  ...
  if (!open) return (
    <button onClick={()=>{ setOpen(true); setText(isEdit ? (ticket.adminResponse||"") : ""); }}
      style={{
        ...
        color:isEdit?"hsl(270,90%,72%)":"hsl(142,80%,68%)",
        transition:"all .15s"
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        <button
          ...
          style={{
            ...
            color: done ? "hsl(142,90%,70%)" : "hsl(270,90%,76%)",
        ...
        >
          {done ? (
            ...
            <><div style={{width:10,height:10,borderRadius:"50%",border:"2px solid hsla(192,100%,55%,0.3)",borderTopColor:"hsl(270,90%,72%)",animation:"spin 0.8s linear infinite"}}/> SENDING...</>
          ) : (
...
Respond ONLY with valid JSON (no markdown):
{
  ...
  const RC: Record<string,string> = {
    ...
    operator:"hsl(270,90%,62%)", analyst:"hsl(158,80%,55%)",
  };
  ...
  const CMDS = [
    ...
    { label:"Database",    sub:"Access Records Vault",     Icon:Database,  path:"/database", hue:195, color:"hsl(270,90%,62%)", shape:"shield"  },
  ...
  return (
    <div style={{minHeight:"100vh",overflow:"hidden",background:"transparent",position:"relative",fontFamily:"'Exo 2',-apple-system,sans-serif"}}>
      ...
      <div style={{
        ...
        height:54,
        zIndex:9999,
        background:"rgba(10,2,28,0.88)",
        borderBottom:"1px solid rgba(150,50,255,0.20)",
        ...
        <div style={{position:"absolute",left:20,top:0,bottom:0,display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:32,height:32,borderRadius:"50%",background:"radial-gradient(circle,rgba(30,130,200,0.4),rgba(10,50,100,0.2))",border:"1.5px solid rgba(50,190,220,0.55)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <Shield style={{width:14,height:14,color:"hsl(270,90%,72%)"}}/>
          </div>
          <div style={{lineHeight:1}}>
            <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:14,fontWeight:900,letterSpacing:"0.12em",color:"rgba(210,242,248,0.98)",textShadow:"0 0 20px rgba(150,50,255,0.5)"}}>BIMS</div>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:7,color:"rgba(150,50,255,0.42)",letterSpacing:"0.1em",textTransform:"uppercase" as const,marginTop:3}}>Biometric Identity Management System</div>
          </div>
        </div>
        ...
        <AnimatePresence>
          {showUserMenu&&(
            <motion.div initial={{opacity:0,y:-8,scale:0.95}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:-8,scale:0.95}} transition={{duration:0.15}}
              style={{position:"fixed",top:60,right:14,minWidth:178,zIndex:10000,padding:"8px 0",borderRadius:14,background:"rgba(3,10,22,0.97)",border:"1px solid rgba(150,50,255,0.22)",boxShadow:"0 20px 60px rgba(0,0,0,0.9)",backdropFilter:"blur(20px)"}}>
              <div style={{padding:"8px 14px 8px",borderBottom:"1px solid rgba(150,50,255,0.12)",marginBottom:4}}>
                ...
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:"rgba(150,50,255,0.45)",marginTop:2}}>@{currentUser?.username}</div>
              </div>
          ...
          <div style={{textAlign:"center" as const}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12,marginBottom:8}}>
              <motion.div
                ...
                <Fingerprint style={{width:24,height:24,color:"hsl(270,90%,60%)",filter:"drop-shadow(0 0 8px rgba(50,190,218,0.8))"}}/>
              </motion.div>
